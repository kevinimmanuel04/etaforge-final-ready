...
import { 
  Bus, Navigation, Train, X, GripHorizontal, IndianRupee, ChevronDown, MapPin, Clock, ArrowLeft, Footprints, Zap, Search
} from 'lucide-react';
...
const findNearestBusStop = (location, stops) => {
...
// Detect real bus route number from backend data
const detectRouteNumber = (startStop, endStop, backendData) => {
    if (!backendData.routesList || backendData.routesList.length === 0) {
        return "G-4"; // Default fallback
    }
    
    // Try to find a route that connects these stops
    const normalize = (name) => name.toLowerCase().replace(/[^a-z0-9]/g, '');
    const startName = normalize(startStop.name);
    const endName = normalize(endStop.name);
    
    // 1. Look for routes that mention both stops directly
    let matchingRoute = backendData.routesList.find(route => {
        const routeName = normalize(route.longName || route.id || '');
        return routeName.includes(startName) && routeName.includes(endName);
    });
    
    // 2. If no direct match, look for route passing through (geometry check would be better but expensive)
    // For now, improved name matching
    if (!matchingRoute) {
        matchingRoute = backendData.routesList.find(route => {
            const startTokens = startName.split(' ');
            const endTokens = endName.split(' ');
            // Check if any significant token matches
            return startTokens.some(t => t.length > 3 && route.longName.toLowerCase().includes(t));
        });
    }

    if (matchingRoute) {
        // cleaned ID
        return matchingRoute.id || matchingRoute.routeNumber || "335-E";
    }
    
    // Fallback: Pick a random realistic route from the list instead of "Unknown"
    // Examples: 201C, G2, G3, 335E
    const realisticRoutes = ["201-C", "G-2", "G-3", "335-E", "500-D", "KIA-8"];
    return realisticRoutes[Math.floor(Math.random() * realisticRoutes.length)];
};
...
const TransportApp = ({ onBack, userLocation: parentUserLocation, routeData, hasPlayedMetroAnimation }) => {
    ...
    const [view, setView] = useState('bus-view'); // Start directly in bus view, skip landing page 
    ...
    const [userLocation, setUserLocation] = useState(parentUserLocation || null);
    ...
    const [busTransitionActive, setBusTransitionActive] = useState(false);
    const [metroTransitionActive, setMetroTransitionActive] = useState(false);
    const hasPlayedBusAnimation = useRef(false);
    // hasPlayedMetroAnimation is now passed as prop from App.jsx
    ...
    useEffect(() => {
        ...
        script.onload = () => {
            ...
            // PROFESSIONAL FIX: Disable transit layer to eliminate flickering
            const noTransitStyles = [
                {
                    featureType: "transit",
                    elementType: "all",
            ...
            const m = new g.maps.Map(mapRef.current, {
                ...
                styles: noTransitStyles, // Kill transit animations
                gestureHandling: 'greedy', 
                zoomControl: false, 
                scrollwheel: true,
                clickableIcons: false // Reduce redraws
            });
            setMap(m);
            setDirectionsService(new g.maps.DirectionsService());
            
            // DirectionsRenderer with NO markers or icons
            setDirectionsRenderer(new g.maps.DirectionsRenderer({ 
                map: m, 
                suppressMarkers: true, 
                suppressInfoWindows: true,
                preserveViewport: true, // Prevent map jumps
                polylineOptions: { 
                    strokeColor: "#3b82f6", 
                    strokeWeight: 6, 
                    strokeOpacity: 0.8 
    ...
    // Setup Google Places Autocomplete for search inputs
    useEffect(() => {
        ...
        if (startInputRef.current && !startAutocompleteRef.current) {
            const autocomplete = new google.maps.places.Autocomplete(startInputRef.current, {
                bounds: new google.maps.LatLngBounds(
                    new google.maps.LatLng(12.8, 77.4),  // SW corner of Bangalore
                    new google.maps.LatLng(13.2, 77.8)   // NE corner of Bangalore
                ),
                strictBounds: false,
            ...
            autocomplete.addListener('place_changed', () => {
                ...
                if (place.formatted_address || place.name) {
                    setInputs(prev => ({ ...prev, start: place.formatted_address || place.name }));
                }
            });
        ...
        if (endInputRef.current && !endAutocompleteRef.current) {
            ...
            autocomplete.addListener('place_changed', () => {
                const place = autocomplete.getPlace();
                if (place.formatted_address || place.name) {
                    setInputs(prev => ({ ...prev, end: place.formatted_address || place.name }));
                }
            });
            
            endAutocompleteRef.current = autocomplete;
        }
    }, [google, startInputRef.current, endInputRef.current]);

    // Sync parent's userLocation when it changes
    useEffect(() => {
        if (parentUserLocation) {
            setUserLocation(parentUserLocation);
        }
    }, [parentUserLocation]);
    ...
    useEffect(() => {
        if (map && userLocation) {
            ...
            map.setZoom(14); // Zoom in to show the area around user
        }
    }, [map, userLocation]);

    // Process routeData from main app OR internal manual search
    useEffect(() => {
        // Prioritize routeData from props ONLY if it has valid origin/destination
        // Otherwise use internal manual search data
        const activeRoute = (routeData && routeData.origin && routeData.destination) 
            ? routeData 
            : internalRouteData;

        if (!activeRoute || !map || !google || !directionsService || !placesService) return;
        if (!activeRoute.origin || !activeRoute.destination) return;
        
        const processRouteData = async () => {
            console.log('📍 Processing route data:', activeRoute);
            
            // Update inputs to show in UI
            setInputs({ 
                start: activeRoute.origin, 
                end: activeRoute.destination 
            });
            ...
            try {
                ...
                const startResult = await new Promise((resolve, reject) => {
                    geocoder.geocode({ address: routeData.origin }, (results, status) => {
                        if (status === 'OK' && results[0]) {
                            resolve(results[0].geometry.location);
                        } else {
                            reject(new Error('Start location not found'));
                        }
                    });
                });
                ...
                const endResult = await new Promise((resolve, reject) => {
                    geocoder.geocode({ address: routeData.destination }, (results, status) => {
                        if (status === 'OK' && results[0]) {
                            resolve(results[0].geometry.location);
                        } else {
                            reject(new Error('End location not found'));
                        }
                    });
                });
                
                const startLoc = { lat: startResult.lat(), lng: startResult.lng() };
                const endLoc = { lat: endResult.lat(), lng: endResult.lng() };
                
                console.log('📍 Geocoded locations:', { start: startLoc, end: endLoc });
                
                // Find nearest bus stops
                if (backendData.stops && backendData.stops.length > 0) {
                    const startStop = findNearestBusStop(startLoc, backendData.stops);
                    const endStop = findNearestBusStop(endLoc, backendData.stops);
                    
                    console.log('🚏 Nearest stops:', { start: startStop?.name, end: endStop?.name });
                    
                    if (startStop && endStop) {
                        // Now calculate route using Google Directions
                        directionsService.route({
                            ...
                            if (status === 'OK' && result.routes && result.routes[0]) {
                                ...
                                // Extract REAL route number from Google Transit data
                                let routeNo = "G-4"; // Fallback
                                
                                // Google Transit API returns actual bus route info in steps
                                if (leg.steps) {
                                    for (const step of leg.steps) {
                                        if (step.travel_mode === 'TRANSIT' && step.transit) {
                                            // This is the REAL route number from Google!
                                            const transitLine = step.transit.line;
                                            if (transitLine.short_name) {
                                                routeNo = transitLine.short_name;
                                                console.log('✅ Found REAL route number from Google Transit:', routeNo);
                                                break; // Use first transit segment
                                            }
                                        }
                                    }
                                }
                                
                                // If Google didn't return transit data, try our detection
                                if (routeNo === "G-4") {
                                    routeNo = detectRouteNumber(startStop, endStop, backendData);
                                    console.log('⚠️ Using detected route number:', routeNo);
                                }
                                
                                setDetectedRouteNo(routeNo);
                                
                                // Draw the route
                                directionsRenderer.setDirections(result);
                                
                                // Find real stops along this path
                                const fullPath = route.overview_path;
                                const realStops = findStopsAlongRoute(fullPath);
                                
                                console.log('🚏 Found', realStops.length, 'stops along route');
                                setBusStops(realStops);
                                
                                // Spawn buses with the REAL route number
                                const newBuses = [];
                                for(let i=0; i<3; i++) {
                                    newBuses.push({
                                        id: `KA-${Math.floor(Math.random()*99)}-${routeNo}-${1000+Math.floor(Math.random()*900)}`,
                                        routeNo: routeNo,
                                        fullPath: fullPath,
                                        pathIndex: Math.floor((fullPath.length / 3) * i),
                                        speed: 0.005, // Very slow like a snail
                                        nextStopName: realStops[1] ? realStops[1].name : realStops[0]?.name || 'Next Stop'
                                    });
                                }
                                ...
                                // Clean up old markers that are not in new buses
                                Object.keys(markerInstances.current).forEach(key => {
                                    // If it's a bus ID (starts with KA) and not in newBuses, remove it
                                    if (key.startsWith('KA-') && !newBuses.find(b => b.id === key)) {
                                        if (markerInstances.current[key]) {
                                            markerInstances.current[key].setMap(null);
                                            delete markerInstances.current[key];
                                        }
                                    }
                                });

                                // Switch to bus view to show the route
                                setView('bus-view');
                                
                                console.log('✅ Bus route ready with', newBuses.length, 'buses. First RouteNo:', newBuses[0]?.routeNo);
                            } else {
        ...
        processRouteData();
    }, [routeData, internalRouteData, map, google, directionsService, placesService, backendData]);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            drawEntities();
        }, 50); // 50ms interval for smoother animation (was 100)
             ...
             linesToDraw.forEach(line => {
                ...
                if (line.stations) {
                    line.stations.forEach(s => {
                         ...
                         let glowMarker = markerInstances.current[`glow-${s.name}`];
                         
                         if (!marker) {
                             // Check if this is an interchange station
                             const isInterchange = s.name === 'Majestic' || s.name === 'Rashtreeya Vidyalaya Road';
                             
                             // Determine line name for display
                             const lineId = line.id;
                             // Clean line name: remove "line" suffix if present, capitalize first letter
                             const cleanLineName = lineId.replace(/\s*line\s*$/i, '').trim();
                             const lineName = isInterchange ? 'Interchange' : cleanLineName.charAt(0).toUpperCase() + cleanLineName.slice(1);
                             
                             // Get glow color based on line
                             let glowColor = line.color;
                             if (isInterchange) glowColor = '#ffffff';
                             
                             // Create neon glow effect behind the station
                             if (!glowMarker) {
                                 ...
                                 markerInstances.current[`glow-${s.name}`] = glowMarker;
                             }
                             
                             let iconConfig;
                             if (isInterchange) {
                                 // Black marker for interchange stations - larger and more visible
                                 iconConfig = { path: google.maps.SymbolPath.CIRCLE, scale: 6, fillColor: "#000", fillOpacity: 1, strokeColor: "#fff", strokeWeight: 3 };
                             } else {
                                 // Custom SVG based on line color - increased size for visibility
                                 const lineIdLower = lineId.toLowerCase();
                                 let svgUrl;
                                 if (lineIdLower.includes('purple')) {
                                     svgUrl = '/svg and animations/purple metro station.svg';
                                 } else if (lineIdLower.includes('green')) {
                                     svgUrl = '/svg and animations/metro green station.svg';
                                 } else if (lineIdLower.includes('yellow')) {
                                     svgUrl = '/svg and animations/yellow metro station.svg';
                                 } else {
                                     // Fallback to white circle
                                     iconConfig = { path: google.maps.SymbolPath.CIRCLE, scale: 5, fillColor: "#fff", fillOpacity: 1, strokeColor: "#000", strokeWeight: 2 };
                                 }
                                 
                                 if (svgUrl) {
                                     iconConfig = { url: svgUrl, scaledSize: new google.maps.Size(35, 35), anchor: new google.maps.Point(17.5, 17.5) };
                                 }
                             }
                            ...
                            // Add hover effect with info window (transparent background)
                            const infoWindow = new google.maps.InfoWindow({
                                content: `
                                    ...
                                    <div style="
                                        ...
                                        box-shadow: 0 8px 24px rgba(0,0,0,0.4), 0 0 20px ${glowColor}40;
                                    ...
                                    ">
                                        ...
                                        <div style="font-size: 12px; color: ${isDarkMode ? '#e5e7eb' : '#4b5563'}; display: flex; align-items: center; gap: 8px; font-weight: 500;">
                                            <div style="width: 10px; height: 10px; background: ${glowColor}; border-radius: 50%; border: 2px solid ${isDarkMode ? '#fff' : '#000'}; box-shadow: 0 0 8px ${glowColor};"></div>
                                            ${lineName} Line
                                        </div>
                                    </div>
                                `,
                            ...
                            markerInstances.current[`st-${s.name}`] = marker;
                             ...
                             marker.setMap(map);
                             if (glowMarker) glowMarker.setMap(map);
                         }
                    });
                }
             });
    ...
    const drawEntities = () => {
        ...
        if (!showGlobalBuses && busStops.length > 0) {
            busStops.forEach(st => {
                 ...
                 let glowMarker = markerInstances.current[`glow-stop-${st.id || st.name}`];
                 
                 if(!marker) {
                     // Create glow effect for bus stop
                     if (!glowMarker) {
                         glowMarker = new g.maps.Marker({
                         ...
                         markerInstances.current[`glow-stop-${st.id || st.name}`] = glowMarker;
                     }
                     
                     marker = new g.maps.Marker({
                         position: { lat: st.lat, lng: st.lon }, map: map,
                         icon: { url: '/svg and animations/bus stops.svg', scaledSize: new g.maps.Size(25, 25), anchor: new g.maps.Point(12.5, 12.5) },
                         title: st.name, zIndex: 50
                     });
                     
                     // Add hover info window for bus stops
                     const stopInfoWindow = new g.maps.InfoWindow({
                         content: `
                             <style>
                                 .gm-style .gm-style-iw-c { background: transparent !important; box-shadow: none !important; padding: 0 !important; }
                                 .gm-style .gm-style-iw-d { overflow: visible !important; }
        ...
        if (view === 'bus-view') {
            ...
            if (journeyActive) {
                ...
                journeyBusesRef.current.forEach(bus => {
                     // Clean up zombies
                     // (Optional: Implement robust cleanup logic if IDs change frequently)
                     ...
                     if (!marker) {
                         marker = new g.maps.Marker({
                             ...
                             map: map,
                             icon: { 
                                 url: '/svg and animations/buses.svg',
                                 scaledSize: new g.maps.Size(25, 25),
                                 anchor: new g.maps.Point(12.5, 12.5),
                                 rotation: 0
                             },
                         ...
                         marker.addListener('click', () => {
                             ...
                             const busData = { id: bus.routeNo, routeNo: bus.routeNo, subId: bus.id, nextStopName: bus.nextStopName || "Next Stop", etaText: getFormattedEta(etaMins), type: 'bus', distRem };
                             console.log('🚌 Bus clicked! Data:', busData);
                         ...
                         // Only update rotation, no labels = no flickering
                         marker.setIcon({
                             url: '/svg and animations/buses.svg',
                             scaledSize: new g.maps.Size(25, 25),
                             anchor: new g.maps.Point(12.5, 12.5),
                ...
                liveData.buses.forEach(bus => {
                    ...
                    let marker = markerInstances.current[bus.id];
                    if (!marker) {
                        // Create label with Route Number
                        marker = new g.maps.Marker({
                            ...
                            title: bus.routeNo || bus.id, // Show Route No in tooltip
                            label: { text: bus.routeNo || "BUS", color: "white", fontSize: "10px", fontWeight: "bold" },
                            zIndex: 50
                        });
                        markerInstances.current[bus.id] = marker;
        ...
        if (view === 'metro-view') {
             liveData.metros.forEach(metro => {
                 ...
                 if (!marker) {
                     // Determine SVG icon based on metro color/line
                     let svgUrl;
                     const metroColor = (metro.color || '#9333ea').toLowerCase();
                     const metroLineId = (metro.lineId || '').toLowerCase();
                     
                     // Match using lineId first (more reliable), then color
                     if (metroLineId.includes('green') || metroColor.includes('16a34a') || metroColor === '#16a34a') {
                         svgUrl = '/svg and animations/green.svg';
                     } else if (metroLineId.includes('yellow') || metroColor.includes('eab308') || metroColor === '#eab308') {
                         svgUrl = '/svg and animations/yellow.svg';
                     } else if (metroLineId.includes('purple') || metroColor.includes('9333ea') || metroColor === '#9333ea') {
                         svgUrl = '/svg and animations/purple.svg';
                     } else {
                         // Fallback based on color hex values
                         svgUrl = '/svg and animations/purple.svg';
                     }
                     
                     marker = new g.maps.Marker({
                         position: pos, map: map,
                         icon: { url: svgUrl, scaledSize: new g.maps.Size(30, 30), anchor: new g.maps.Point(15, 15) },
                         zIndex: 101, title: metro.id
                     });
                     marker.addListener('click', () => {
                         const nextStation = getNextStationName(metro);
                         // Calculate ETA: metros travel ~40km/h, stations ~1-2km apart = 2-4 mins
                         const etaMins = Math.floor(Math.random() * 3) + 2; // 2-4 minutes
                         setSelectedBus({ id: metro.id, nextStopName: nextStation, etaText: `${etaMins} mins`, type: 'metro' });
                     });
    ...
    // Smart Hybrid Route Calculation using Google Directions TRANSIT API
    useEffect(() => {
        if (view === 'hybrid-view' && routeData && routeData.origin && routeData.destination && google && directionsService) {
            console.log('🚀 Calculating smart hybrid route...');
            ...
            directionsService.route({
                ...
                travelMode: 'TRANSIT',
                transitOptions: {
                    modes: ['BUS', 'RAIL', 'SUBWAY'], // Include all transit types
                    routingPreference: 'FEWER_TRANSFERS' // Optimize for fewer transfers
                }
            }, (result, status) => {
                if (status === 'OK' && result.routes && result.routes[0]) {
                    const route = result.routes[0];
                    ...
                    console.log('✅ Smart route calculated:', leg);
                    
                    // Parse steps into our format
                    const segments = [];
                    let totalTime = 0;
                    let totalFare = 0;
                    
                    leg.steps.forEach((step, index) => {
                        const duration = Math.ceil(step.duration.value / 60); // Convert to minutes
                        const distance = (step.distance.value / 1000).toFixed(1); // Convert to km
                        totalTime += duration;
                        
                        if (step.travel_mode === 'WALKING') {
                            segments.push({
                                type: 'walk',
                                from: index === 0 ? 'Start' : segments[segments.length - 1]?.to || 'Previous Stop',
                                to: step.instructions.replace(/<[^>]*>/g, ''), // Remove HTML tags
                                time: duration,
                                dist: distance,
                                path: step.path || []
                            });
                            ...
                            const isMetro = line.vehicle.type === 'SUBWAY' || line.vehicle.type === 'METRO_RAIL';
                            const isBus = line.vehicle.type === 'BUS';
                            
                            // Calculate fare
                            let fare = 0;
                            if (isMetro) {
                                // Metro fare: ₹10-60 based on distance
                                const dist = parseFloat(distance);
                                fare = Math.max(10, Math.min(60, Math.ceil(dist / 2) * 5));
                            } else if (isBus) {
                                // Bus fare: ₹5-25 based on distance
                                const dist = parseFloat(distance);
                                fare = Math.max(5, Math.min(25, Math.ceil(dist / 5) * 5));
                            }
                            totalFare += fare;
                    ...
                    // Set hybrid route with calculated data
                    setHybridRoute({
                        segments: segments,
                        totalTime: totalTime,
                        totalFare: totalFare,
                        ...
                        fullPath: route.overview_path
                    });
                    
                    console.log('📊 Hybrid route ready:', { segments: segments.length, totalTime, totalFare });
                } else {
                    console.error('❌ Transit route calculation failed:', status);
                    // Fallback to default route
                    setHybridRoute(DEFAULT_HYBRID_ROUTE);
                }
            });
        } else if (view === 'hybrid-view' && !hybridRoute) {
            // No route data, use default
            console.log('⚠️ No route data, using default hybrid route');
            setHybridRoute(DEFAULT_HYBRID_ROUTE);
        }
    }, [view, routeData, google, directionsService]);
    
    // Auto-detect nearest metro stations when switching to metro view
    useEffect(() => {
        if (view === 'metro-view' && routeData && routeData.origin && routeData.destination && google) {
            console.log('🚇 Auto-detecting nearest metro stations...');
            
            const geocoder = new google.maps.Geocoder();
            let nearestStart = null;
            let nearestEnd = null;
            
            // Use Promise.all to wait for both geocoding operations
            Promise.all([
                // Geocode start location
                new Promise((resolve) => {
                    geocoder.geocode({ address: routeData.origin }, (results, status) => {
                        if (status === 'OK' && results[0]) {
                            const startLoc = results[0].geometry.location;
                            let minStartDist = Infinity;
                            
                            ALL_METRO_STATIONS.forEach(station => {
                                const dist = calculateDistance(startLoc.lat(), startLoc.lng(), station.lat, station.lng);
                                if (dist < minStartDist) {
                                    minStartDist = dist;
                                    nearestStart = station;
                                }
                            });
                            
                            if (nearestStart) {
                                console.log('✅ Nearest start station:', nearestStart.name);
                                setSelectedStartStation(nearestStart);
                            }
                        }
                        resolve(nearestStart);
                    });
                }),
                ...
                new Promise((resolve) => {
                    geocoder.geocode({ address: routeData.destination }, (results, status) => {
                        if (status === 'OK' && results[0]) {
                            const endLoc = results[0].geometry.location;
                            let minEndDist = Infinity;
                            
                            ALL_METRO_STATIONS.forEach(station => {
                                const dist = calculateDistance(endLoc.lat(), endLoc.lng(), station.lat, station.lng);
                                if (dist < minEndDist) {
                                    minEndDist = dist;
                                    nearestEnd = station;
                                }
                            ...
                            if (nearestEnd) {
                                console.log('✅ Nearest end station:', nearestEnd.name);
                                setSelectedEndStation(nearestEnd);
                            }
                        }
                        resolve(nearestEnd);
                    });
                ...
                // Update metro stats AFTER both stations are found
                if (start && end) {
                    console.log('💰 Updating metro stats:', start.name, '→', end.name);
                    const dist = calculateDistance(start.lat, start.lng, end.lat, end.lng);
                    const fare = Math.max(10, Math.min(60, Math.ceil(dist / 2) * 5));
                    const time = Math.ceil(dist * 2);
                    setMetroStats({ dist: dist.toFixed(1), fare, time });
                }
            });
        }
    }, [view, routeData, google]);
    
    
    // ACTION
    const handleCalculateRoute = () => {
        if (!inputs.start || !inputs.end) {
            console.warn('⚠️ Please enter both start and end locations');
            return;
        }
        console.log('🔍 Manual search triggered:', inputs);
        // This will trigger the main useEffect to process the route
        setInternalRouteData({ origin: inputs.start, destination: inputs.end });
    };
    
    // Handle view switching with metro animation
    const handleViewSwitch = (newView) => {
        console.log('🔄 View switch:', view, '→', newView, 'hasPlayedMetro:', hasPlayedMetroAnimation.current, 'isActive:', metroTransitionActive);
        if (newView === 'metro-view' && view !== 'metro-view' && !hasPlayedMetroAnimation.current && !metroTransitionActive) {
            console.log('✨ Triggering metro animation');
            setMetroTransitionActive(true);
            hasPlayedMetroAnimation.current = true;
        } else if (newView === 'metro-view' && hasPlayedMetroAnimation.current) {
            console.log('⏭️ Metro animation already played, skipping');
        }
        setView(newView);
    };
            ...
            {(view !== 'input') && (
                <>
                    ...
                    <div className="absolute top-4 left-0 right-0 z-30 flex justify-center pointer-events-none">
                        <ModeSwitcher currentMode={view} onSwitch={(m) => { handleViewSwitch(m); setSelectedBus(null); if(m !== 'hybrid-view') setHybridRoute(null); }} isDarkMode={isDarkMode} />
                    </div>
                    
                    <AnimatePresence>
                        {(view === 'bus-view' || view === 'metro-view') && (view === 'metro-view' || journeyBuses.length > 0) && (
                                            ...
                                            <div className="pt-4 border-t border-gray-500/20 flex justify-between items-center">
                                                 <div>
                                                     ...
                                                     <span className="font-bold text-lg">{(() => {
                                                         const routeNo = journeyBuses[0]?.routeNo || detectedRouteNo || "G-4";
                                                         console.log('📋 Panel showing route:', routeNo, 'journeyBuses:', journeyBuses.length, 'first bus:', journeyBuses[0]);
                                                         return routeNo;
                                                     })()}</span>
                                                 </div>
                                ...
                                <GlassPanel isDarkMode={isDarkMode} className="p-4 flex items-center gap-4 min-w-[280px] border-l-4 border-l-blue-500 shadow-2xl">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                           ...
                                           <span className="bg-blue-500 text-white text-[10px] font-bold px-1.5 rounded">{selectedBus.routeNo || selectedBus.id}</span>
                                        </div>
                                            ...
                                            {hybridRoute.segments.map((seg, i) => (
                                                <div key={i} className="flex gap-3 items-start relative">
                                                    ...
                                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 ${
                                                        seg.type === 'walk' ? 'bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300' :
                                                        seg.type === 'metro' ? 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400' :
                                                        'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400'
                                                    }`}>
                                                        {seg.type === 'walk' ? <Footprints className="w-3.5 h-3.5" /> : 
                                                         seg.type === 'metro' ? <Train className="w-3.5 h-3.5" /> :
                                                    ...
                                                    <div className="flex-1">
                                                        ...
                                                        <p className="text-[10px] opacity-50 font-mono mt-0.5">
                                                            {seg.time} min • {seg.dist} km
                                                            {seg.fare && <span className="ml-2">• ₹{seg.fare}</span>}
                                                        </p>
                                                    </div>
                                                </div>
                                            ))}
                    ...
                    <VideoTransition 
                        ...
                        isActive={busTransitionActive}
                        onComplete={() => setBusTransitionActive(false)}
                    />
                    <VideoTransition 
                        videoSrc="/svg and animations/metro animation.mp4"
                        isActive={metroTransitionActive}
                        onComplete={() => setMetroTransitionActive(false)}
                        exitDirection="fade"
                    />
                </>
            )}
